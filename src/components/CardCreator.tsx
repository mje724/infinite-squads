...
import { 
  ...
  ChevronDown,
  Plus
cat > src/components/CardCreator.tsx << 'ENDOFFILE'
'use client';
        ...
        <div className="absolute inset-[3px] rounded-xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-16 flex items-center justify-between px-4 z-20">
            ...
            {currentCard.position && (
              <div className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider" style={{ background: `${rarityStyle.border}20`, color: rarityStyle.border, border: `1px solid ${rarityStyle.border}40` }}>{currentCard.position}</div>
            )}
          ...
          <div className="absolute top-14 left-0 right-0 h-48 overflow-hidden">
            {currentCard.image ? (
              <div className="w-full h-full bg-cover bg-center" style={{ backgroundImage: `url(${currentCard.image})`, filter: getFilterStyle(currentCard.imageFilter || 'normal') }} />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-slate-800/50"><User className="w-20 h-20 text-slate-600" /></div>
            )}
          ...
          {(currentCard.traits || []).length > 0 && (
            <div className="absolute bottom-16 left-4 right-4 flex gap-2 justify-center flex-wrap">
              {(currentCard.traits || []).slice(0, 4).map((traitId) => {
                ...
                return <div key={traitId} className="w-8 h-8 rounded-full bg-slate-700/80 flex items-center justify-center text-lg" title={trait.name}>{trait.icon}</div>;
              })}
            </div>
          )}
          {currentCard.bio && (
            <div className="absolute bottom-4 left-4 right-4">
              <p className="text-[10px] text-slate-400 italic text-center" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', wordWrap: 'break-word' }}>&ldquo;{currentCard.bio}&rdquo;</p>
            </div>
          )}
          ...
          {currentCard.rarity === 'glitch' && (
            <motion.div className="absolute inset-0 pointer-events-none mix-blend-screen" style={{ background: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,255,255,0.03) 2px, rgba(0,255,255,0.03) 4px)' }} animate={{ y: [0, -4, 0] }} transition={{ duration: 0.1, repeat: Infinity }} />
          )}
        </div>
      </div>
    </motion.div>
...
const StatSlider: React.FC<StatSliderProps> = ({ stat, index, onUpdate, onRemove, availableStats }) => {
    ...
    <div className="group relative bg-slate-800/50 rounded-lg p-3 border border-slate-700/50 hover:border-slate-600 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <div className="relative flex-1">
          <button onClick={() => setIsSelectOpen(!isSelectOpen)} className="flex items-center gap-2 text-sm font-medium text-slate-200 hover:text-white transition-colors">
            <span className="text-lg">{stat.icon}</span><span>{stat.label}</span><ChevronDown className="w-3 h-3 text-slate-400" />
          </button>
          <AnimatePresence>
            {isSelectOpen && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full left-0 mt-1 w-56 max-h-48 overflow-y-auto bg-slate-900 rounded-lg border border-slate-700 shadow-xl z-50">
                {availableStats.map((preset) => (
                  <button key={preset.id} onClick={() => { onUpdate(index, { label: preset.label, icon: preset.icon, category: preset.category }); setIsSelectOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white transition-colors text-left">
                    <span>{preset.icon}</span><span>{preset.label}</span><span className="ml-auto text-xs text-slate-500">{preset.category}</span>
                  </button>
                ))}
              </motion.div>
            )}
...
const ModeToggle: React.FC = () => {
  ...
  return (
    <div className="flex items-center justify-center gap-2 p-1 bg-slate-800/50 rounded-full border border-slate-700">
      <button onClick={() => setMode('serious')} className={cn('px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300', mode === 'serious' ? 'bg-gradient-to-r from-amber-500 to-yellow-500 text-black shadow-lg' : 'text-slate-400 hover:text-white')} style={mode === 'serious' ? { boxShadow: '0 0 20px rgba(245, 158, 11, 0.25)' } : {}}>
        <span className="flex items-center gap-2"><Trophy className="w-4 h-4" />Serious Mode</span>
      </button>
      <button onClick={() => setMode('unserious')} className={cn('px-6 py-2 rounded-full text-sm font-semibold transition-all duration-300', mode === 'unserious' ? 'bg-gradient-to-r from-cyan-500 to-pink-500 text-black shadow-lg' : 'text-slate-400 hover:text-white')} style={mode === 'unserious' ? { boxShadow: '0 0 20px rgba(236, 72, 153, 0.25)' } : {}}>
        <span className="flex items-center gap-2"><Zap className="w-4 h-4" />Unserious Mode</span>
      </button>
    </div>
  );
};
...
export default function CardCreator() {
  ...
  const handleImageUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => { setImageToCrop(event.target?.result as string); setShowCropper(true); };
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-start">
          <div className="space-y-6">
            <motion.div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
            ...
            <motion.div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            ...
            <motion.div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }}>
            ...
            <motion.div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 }}><TraitSelector /></motion.div>
            <motion.div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
