...
// ============================================
// INFINITE SQUADS - Card Creator Component
// "God Mode" Card Builder with Live Preview
// ============================================
...
const CardPreview: React.FC = () => {
  const { currentCard, mode, calculateOverallRating } = useCardCreator();
        ...
        <div className="absolute inset-[3px] rounded-xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden">
          ...
          <div className="absolute top-56 left-0 right-0 text-center px-4">
            ...
            {currentCard.nickname && (
              <p className="text-sm text-slate-400 italic">
                &ldquo;{currentCard.nickname}&rdquo;
              </p>
            )}
          </div>
              ...
              {(currentCard.statBlock || []).slice(0, 6).map((stat, i) => (
                <div key={stat.id || i} className="flex items-center justify-between">
                  <span className="text-xs text-slate-400 flex items-center gap-1">
                    <span>{stat.icon}</span>
                    <span className="uppercase tracking-wider">{stat.label.slice(0, 8)}</span>
                  </span>
                  <span 
                    className="text-sm font-bold"
                    style={{ 
                      color: stat.value >= 80 ? '#22c55e' : 
                             stat.value >= 60 ? '#eab308' : 
                             stat.value >= 40 ? '#f97316' : '#ef4444',
                    }}
                  >
                    {stat.value}
                  </span>
            ...
            <div className="absolute bottom-16 left-4 right-4 flex gap-2 justify-center">
              {(currentCard.traits || []).slice(0, 4).map((traitId) => {
                ...
                return (
                  <div
                    key={traitId}
                    className="w-8 h-8 rounded-full bg-slate-700/80 flex items-center justify-center text-lg"
                    title={trait.name}
                  >
                    {trait.icon}
                  </div>
          ...
          {currentCard.bio && (
            <div className="absolute bottom-4 left-4 right-4">
              <p className="text-[10px] text-slate-400 italic text-center line-clamp-2">
                &ldquo;{currentCard.bio}&rdquo;
              </p>
            </div>
          )}
          
          {/* Holographic Overlay for Holo/Glitch */}
...
interface StatSliderProps {
  stat: Stat;
  index: number;
  onUpdate: (index: number, updates: Partial<Stat>) => void;
  onRemove: (index: number) => void;
  availableStats: typeof STAT_PRESETS;
}
    ...
    <div className="group relative bg-slate-800/50 rounded-lg p-3 border border-slate-700/50 hover:border-slate-600 transition-colors">
      ...
      <div className="flex items-center justify-between mb-2">
          ...
          <AnimatePresence>
            {isSelectOpen && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute top-full left-0 mt-1 w-56 max-h-48 overflow-y-auto bg-slate-900 rounded-lg border border-slate-700 shadow-xl z-50"
              >
                {availableStats.map((preset) => (
                  <button
                    key={preset.id}
                    onClick={() => {
                      onUpdate(index, { 
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white transition-colors text-left"
                  >
                    <span>{preset.icon}</span>
                    <span>{preset.label}</span>
                    <span className="ml-auto text-xs text-slate-500">{preset.category}</span>
                  </button>
                ))}
              </motion.div>
            )}
        ...
        <div className="flex items-center gap-2">
          <span className="text-lg font-bold text-white w-8 text-right" style={{ fontVariantNumeric: 'tabular-nums' }}>
            {stat.value}
          </span>
          <button
      ...
      <div className="relative h-2 bg-slate-700 rounded-full overflow-hidden">
        <motion.div
          className={cn('h-full rounded-full', getValueColor(stat.value))}
          initial={{ width: 0 }}
  ...
  return (
    <div className="flex items-center justify-center gap-2 p-1 bg-slate-800/50 rounded-full border border-slate-700">
      <button
        ...
        style={mode === 'serious' ? { boxShadow: '0 0 20px rgba(245, 158, 11, 0.25)' } : {}}
      >
      ...
      <button
        ...
        style={mode === 'unserious' ? { boxShadow: '0 0 20px rgba(236, 72, 153, 0.25)' } : {}}
      >
        <span className="flex items-center gap-2">
...
const TraitSelector: React.FC = () => {
  ...
  const selectedTraits = (currentCard.traits || []).map(
    (id) => TRAIT_PRESETS.find((t) => t.id === id)
  ).filter(Boolean);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {selectedTraits.map((trait) => trait && (
          <motion.div
            key={trait.id}
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            ...
            className={cn(
              ...
              trait.type === 'positive' && 'bg-green-500/10 border-green-500/30 text-green-400',
              trait.type === 'negative' && 'bg-red-500/10 border-red-500/30 text-red-400',
              trait.type === 'neutral' && 'bg-slate-500/10 border-slate-500/30 text-slate-400',
              trait.type === 'chaotic' && 'bg-purple-500/10 border-purple-500/30 text-purple-400'
            )}
          >
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 rounded-full text-sm border border-dashed border-slate-600 text-slate-400 hover:border-slate-500 hover:text-slate-300 transition-colors"
        >
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-slate-900 rounded-2xl border border-slate-700 p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto"
            ...
            >
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="w-full mt-4 py-3 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg font-semibold text-white hover:opacity-90 transition-opacity"
              >
                Done
              </button>
            </motion.div>
...
export default function CardCreator() {
  ...
  const availableStats = mode === 'serious' 
    ? STAT_PRESETS.filter((s) => s.category === 'sports' || s.category === 'office')
    : STAT_PRESETS.filter((s) => s.category === 'roast' || s.category === 'party');
  
  // Get positions based on mode
  const positions = mode === 'serious'
    ? [...(POSITION_PRESETS.sports || []), ...(POSITION_PRESETS.office || [])]
    : [...(POSITION_PRESETS.party || []), ...(POSITION_PRESETS.roast || [])];
  ...
  // Initialize with preset stats when mode changes
  useEffect(() => {
    if ((currentCard.statBlock || []).length === 0) {
      loadPresetStats(mode === 'serious' ? 'sports' : 'roast');
    }
  }, [mode, currentCard.statBlock, loadPresetStats]);
  ...
  // Add new stat
  const handleAddStat = () => {
    const randomPreset = availableStats[Math.floor(Math.random() * availableStats.length)];
    addStat({
      id: nanoid(),
      label: randomPreset.label,
      value: randomPreset.defaultValue || 50,
      icon: randomPreset.icon,
      category: randomPreset.category,
    });
  };

  const rarityOptions: Rarity[] = mode === 'serious' 
    ? ['bronze', 'silver', 'gold', 'legendary']
    : ['silver', 'gold', 'holo', 'glitch', 'legendary'];
    
  const filterOptions: { value: ImageFilter; label: string }[] = mode === 'serious'
    ? [
    ...
    : [
        ...
        { value: 'security', label: '📹 Security Cam' },
        { value: 'vhs', label: '📼 VHS' },
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {/* Nickname */}
                <div>
                  <label className="text-sm font-medium text-slate-300 mb-1 block">Nickname (Optional)</label>
                  <input
                    type="text"
                    value={currentCard.nickname || ''}
                    onChange={(e) => setNickname(e.target.value)}
                    placeholder="The GOAT, Big Money, etc..."
                    className="w-full px-4 py-3 bg-slate-900/50 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
                  />
                </div>
                ...
                {/* Position */}
                <div>
                  <label className="text-sm font-medium text-slate-300 mb-1 block">Position</label>
                  <select
            ...
            >
              ...
              <div 
                ...
                className="relative h-32 border-2 border-dashed border-slate-600 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-cyan-500 transition-colors group"
              >
                {currentCard.image ? (
              ...
              {/* Filter Options */}
              <div className="mt-4">
                <label className="text-sm font-medium text-slate-300 mb-2 block">Image Filter</label>
                <div className="flex flex-wrap gap-2">
                  {filterOptions.map((option) => (
                    <button
                      ...
                      className={cn(
                        ...
                        currentCard.imageFilter === option.value
              ...
              <div className="flex flex-wrap gap-2">
                {rarityOptions.map((rarity) => (
                  <button
                    ...
                    className={cn(
                      'px-4 py-2 rounded-lg text-sm font-semibold border transition-all capitalize',
                      currentCard.rarity === rarity
                        ? 'text-white'
                        : 'bg-slate-800/50 border-slate-700 text-slate-400 hover:border-slate-600'
                    )}
                    style={currentCard.rarity === rarity ? {
                      background: RARITY_STYLES[rarity].gradient,
                      borderColor: RARITY_STYLES[rarity].border,
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => loadPresetStats(mode === 'serious' ? 'sports' : 'roast')}
                  ...
                  {(currentCard.statBlock || []).length < 8 && (
                    <button
              ...
              <div className="space-y-3">
                {(currentCard.statBlock || []).map((stat, index) => (
                  <StatSlider
                    key={stat.id}
                    stat={stat}
                    index={index}
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-white">Bio / Quote</h3>
                <button
                  onClick={() => randomizeQuote(mode === 'serious' ? 'serious' : 'funny')}
                  className="flex items-center gap-2 px-3 py-1.5 bg-purple-500/20 hover:bg-purple-500/30 rounded-lg text-purple-400 text-sm transition-colors"
                >
              ...
              <textarea
                ...
                placeholder="Add a bio, quote, or roast..."
                rows={3}
            ...
            >
              <button
                onClick={resetCard}
                className="flex-1 flex items-center justify-center gap-2 py-4 bg-slate-800/50 hover:bg-slate-800 border border-slate-700 rounded-xl text-slate-300 font-semibold transition-colors"
              >
              ...
              <div className="mt-6 flex items-center gap-6 text-sm">
                <div className="text-center">
                  <div className="text-2xl font-bold text-cyan-400">
                    {(currentCard.statBlock || []).length}
                  </div>
                  <div className="text-slate-500">Stats</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-400">
                    {(currentCard.traits || []).length}
                  </div>
                  <div className="text-slate-500">Traits</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold capitalize" style={{ color: RARITY_STYLES[currentCard.rarity || 'gold'].border }}>
                    {currentCard.rarity || 'Gold'}
                  </div>
                  <div className="text-slate-500">Rarity</div>
                </div>
              </div>
            </motion.div>
          </div>
