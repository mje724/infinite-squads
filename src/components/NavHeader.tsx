...
export default function NavHeader() {
  ...
  // Count pack cards (mode: 'unserious')
  const packCardCount = cards.filter(c => c.mode === 'unserious').length;
    ...
    <nav className="fixed top-0 left-0 right-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/battle" className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-500 to-orange-500 border border-red-400 rounded-lg text-white text-sm font-semibold hover:opacity-90 transition-colors">
            <Swords className="w-4 h-4" /><span className="hidden sm:inline">Battle</span>
          </Link>
          <Link href="/my-cards" className="flex items-center gap-2 px-4 py-2 bg-slate-800/50 hover:bg-slate-800 border border-slate-700 rounded-lg text-slate-300 hover:text-white text-sm font-medium transition-colors">
            <Users className="w-4 h-4" /><span className="hidden sm:inline">My Cards</span>
            {packCardCount > 0 && <span className="px-2 py-0.5 bg-cyan-500/20 text-cyan-400 text-xs rounded-full">{packCardCount}</span>}
          </Link>
