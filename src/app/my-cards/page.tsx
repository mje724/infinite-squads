          ...
          <AnimatePresence>
            {showActions && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/70 flex items-center justify-center gap-3">
                <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-3 bg-red-500/20 hover:bg-red-500/40 rounded-full text-red-400 transition-colors"><Trash2 className="w-5 h-5" /></button>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
        ...
        {cards.length > 0 ? (
          <motion.div layout className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            <AnimatePresence>{cards.map((card) => <MiniCard key={card.id} card={card} onDelete={() => removeCard(card.id)} />)}</AnimatePresence>
          </motion.div>
        ) : (
